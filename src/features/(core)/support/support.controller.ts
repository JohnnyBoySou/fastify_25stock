...
export const SupportController = {
  ...
  async findByQuery(request: FindByQuerySupportRequest, reply: FastifyReply) {
    try {
  ...
  async findMessagesByTicket(request: FindMessagesByTicketRequest, reply: FastifyReply) {
    try {
      const { id: ticketId } = request.params
      const { page = 1, limit = 10 } = request.query
      ...
      const result = await SupportQueries.findMessagesByTicketId(ticketId, page, limit)
